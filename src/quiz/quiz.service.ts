...
export class QuizService {
  ...
  async createTest(userId: string, dto: CreateTestDto) {
    ...
    return this.prisma.test.create({
      data: {
        ...
        timeLimit:
          typeof dto.timeLimit === 'string'
            ? parseInt(dto.timeLimit) || 10
            : dto.timeLimit || 10,
  ...
  async addQuestion(
    ...
    if (test.creatorId !== userId)
      throw new ForbiddenException('Not allowed to add questions to this test')
    ...
    const questionData = {
      ...
      image: imagePath,
      weight:
        typeof dto.weight === 'string'
          ? parseInt(dto.weight) || 1
          : dto.weight || 1,
    }
  ...
  async updateQuestion(
    ...
    if (question.test.creatorId !== userId)
      throw new ForbiddenException('Not allowed to edit this question')
    ...
    const updateData = {
      ...
      ...(questionData.weight && {
        weight:
          typeof questionData.weight === 'string'
            ? parseInt(questionData.weight) || 50
            : questionData.weight,
      }),
  ...
  async startPracticeTest(userId: string, testId: string) {
    ...
    const firstQuestion = test.questions.sort((a, b) =>
  ...
  async submitPracticeTest(
    ...
    const attemptAnswers = existingAnswers.map(
      ({ questionId, selectedAnswers, userAnswer }) => {
  ...
  async submitExamTest(
    ...
    const finalScore =
      attemptStatus === AttemptStatus.TIMEOUT ? Math.max(0, score - 10) : score
    ...
    const detailedResults: DetailedTestResult[] | null = attempt.test
      .showAnswers
    ...
    const mappedResults: DetailedTestResult[] = results.map((r) => ({
      ...
      userAnswer:
        r.question.type === 'SHORT_ANSWER' ||
          r.question.type === 'OPEN_QUESTION'
          ? r.userAnswer
          : null,
      isCorrect:
        r.question.type === 'OPEN_QUESTION' && attempt.test.examMode
          ? null
          : r.isCorrect,
  ...
  async getPracticeTestResults(
    ...
    return this.getTestResults(
      attemptId,
    ) as Promise<PracticeTestResultsResponse>
  }
  ...
  async exportCompletedTestToPDF(attemptId: string, res: Response) {
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename=test-results-${attempt.id}.pdf`,
    )
    doc.pipe(res)
    ...
    doc
      .fontSize(20)
      .text(`Результаты теста: ${attempt.test.title}`, { align: 'center' })
    doc.moveDown()
    doc
      .fontSize(12)
      ...
      .text(
        `Дата: ${attempt.endTime ? new Date(attempt.endTime).toLocaleString() : 'Не завершен'}`,
      )
    ...
    attempt.answers.forEach((answer, index) => {
      ...
      switch (answer.question.type) {
        ...
        case 'SHORT_ANSWER':
          doc
            ...
            .fillColor('green')
            .text(
              `Правильный ответ: ${answer.question.correctAnswers.join(' или ')}`,
            )
          break
        ...
        case 'TRUE_FALSE':
          doc
            ...
            .fillColor('blue')
            .text(
              `Ответ студента: ${answer.selectedAnswers[0] || 'Нет ответа'}`,
            )
            .fillColor('green')
      ...
      if (
        ...
        doc.text(
          `Получено баллов: ${answer.isCorrect ? answer.question.weight || 1 : 0}`,
        )
      }
    ...
    const pendingAnswers = attempt.answers.filter(
      (a) => a.question.type === 'OPEN_QUESTION' && a.status !== 'CHECKED',
    ).length
  ...
  async exportTestToPDF(testId: string, res: Response) {
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename=test-${test.id}.pdf`,
    )
    doc.pipe(res)
  ...
  async exportTestWithAnswersToPDF(testId: string, res: Response) {
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename=test-with-answers-${test.id}.pdf`,
    )
    doc.pipe(res)
    ...
    doc
      .fontSize(20)
      .text(`Тест: ${test.title} (с ответами)`, { align: 'center' })
    doc.moveDown()
    doc
      .fontSize(12)
    ...
    test.questions.forEach((question, index) => {
      ...
      doc
        ...
        .fillColor('gray')
        .text(
          `Тип вопроса: ${question.type === 'MULTIPLE_CHOICE'
            ? 'Множественный выбор'
